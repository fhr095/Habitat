...
import { objectMapper } from "./utils/ObjectReferenceMapper"; // Ajuste o caminho conforme necessário
...
function SceneContent({ habitatId, mainFileUrl, mobileFileUrl }) {
  ...
  useEffect(() => {
    ...
    if (scene && currentMode) {
      ...
      window._debugSketchMode = {
        getSketchStatus: () => ({ currentMode }),
        // Outras funções de debug...
      };
    }
  }, [scene, currentMode]);
  ...
  useEffect(() => {
    ...
    const initTimeout = setTimeout(() => {
      ...
      if (currentMode === 'sketch' && sketchGroup) {
        ...
        // Se retornamos ao modo normal e temos modelos carregados,
        // podemos verificar e corrigir estados de visibilidade
        if (model2Ref.current) {
          model2Ref.current.traverse((child) => {
            if ((child.isMesh || child.isGroup) && 
                child.userData && 
                child.userData._hiddenBySketchMode) {
              // Restaura a visibilidade
              child.visible = child.userData._originalVisibility !== undefined
                ? child.userData._originalVisibility
                : true;
  ...
  }, [currentMode, model2Ref.current, scene]);
