...
const LocationLabel = ({ world, modelRef, targetMeshName = "Você_está_aqui" }) => {
  ...
  // Configurações de cor e estilo consistentes
  const COLOR_PRIMARY = 0x4e5aff; // Azul do balão - convertido para hex
  ...
  useEffect(() => {
    ...
    const lineMaterial = new THREE.LineBasicMaterial({ 
      ...
      depthTest: false // Para garantir que a linha fique sempre visível
    });
    ...
    line.renderOrder = 999; // Garantir que seja renderizado por último (acima do modelo)
    ...
    pulse.renderOrder = 1000; // Garantir que seja renderizado acima da linha
    ...
    const updateLabelPosition = () => {
      ...
      // Ajustar posição baseado no ângulo de visualização e no tamanho atual do rótulo
      if (dot < -0.5) {
      ...
      // Limita o tamanho máximo para evitar que o texto ultrapasse o balão
      const scale = Math.max(0.5, Math.min(1.2, 40 / distance));
      ...
      // Usar interpolação suave para o ponto final se já temos um ponto anterior
      let finalEndPoint = endPoint;
      if (prevEndPointRef.current) {
        // Interpolar suavemente entre o ponto final anterior e o atual
      ...
      // Ajustar curva mais suavemente com base na distância e posição da câmera
      ...
      // Atualizar a geometria da linha com a curva - usando mais pontos para suavidade
