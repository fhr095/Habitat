...
const SketchRenderer = ({ modelRef, scene, camera, renderer, world }) => {
  ...
  const mergeObject = (object) => {
    ...
    try {
      ...
      if (youAreHereMarker) {
        ...
        console.log("Você está aqui encontrado:", youAreHereMarker.name);
        console.log("Posição global:", youAreHereRef.current.position);
      }
      ...
      // Mesclar geometrias sem alterar posição
      ...
      // IMPORTANTE: NÃO centralizar a geometria, isso estava causando o deslocamento
      // mergedGeometry.center(); // Remover esta linha!
      
      const group = new THREE.Group();
      const mesh = new THREE.Mesh(mergedGeometry);
      group.add(mesh);
      
      // Criar um marcador "Você está aqui" no grupo mesclado, se necessário
  ...
  // Restaura a visibilidade original dos objetos
  ...
  // Função para ocultar os modelos originais durante o modo sketch
  const hideOriginalModels = () => {
    // Primeiro, salva o estado original
    saveOriginalVisibility();
    
    // Agora, oculta o modelo original
    if (modelRef.current) {
  ...
  useEffect(() => {
    console.log("Modo de visualização alterado:", currentMode);
