// SketchRenderer.jsx - Correção do problema de propriedade indefinida e glitches durante transições de câmera
...
window._sketchModeGlobalCache = window._sketchModeGlobalCache || {
  ...
  cameraTransitionActive: false // Nova propriedade para rastrear transições de câmera
};
...
window._debugSketchMode = {
  ...
  // Nova função para iniciar/parar um modo de transição de câmera
  setCameraTransition: (active) => {
    GlobalCache.cameraTransitionActive = active;
    console.log(`Modo de transição de câmera ${active ? 'ATIVADO' : 'DESATIVADO'}`);
    return active;
  }
};
...
function forceModelVisibility(model) {
  ...
  model.traverse(child => {
    if (child.isMesh || child.isGroup) {
      ...
      if (child.isMesh && GlobalCache.originalMaterials && 
          GlobalCache.originalMaterials.has && GlobalCache.originalMaterials.has(child.uuid)) {
...
const SketchRenderer = ({ modelRef, scene, camera, renderer, world }) => {
  ...
  // Flag para indicar se estamos em modo de transição de câmera
  ...
  // Monitorar as transições de câmera
  useEffect(() => {
    const checkCameraTransition = () => {
      cameraTransitionRef.current = GlobalCache.cameraTransitionActive;
      
      // Se estamos em transição de câmera, aplicar otimizações específicas
      if (cameraTransitionRef.current && currentMode === 'sketch') {
        // Simplificar o renderizado durante transições
        simplifyDuringTransition();
      }
    };
    
    // Verificar a cada 100ms
    const intervalId = setInterval(checkCameraTransition, 100);
    
    return () => clearInterval(intervalId);
  }, [currentMode]);
  
  // Função para simplificar o modo sketch durante transições de câmera
  const simplifyDuringTransition = () => {
    const { sketchGroup } = sketchSceneRef.current;
    if (!sketchGroup) return;
    
    // Temporariamente simplificar materiais e ocultar efeitos complexos
    sketchGroup.traverse(obj => {
      // Reduzir resolução de materiais LineSegments2
      if ((obj.isLineSegments2 || obj.type === 'LineSegments2') && 
          obj.material && obj.material.resolution) {
        // Armazenar resolução original se ainda não estiver armazenada
        if (!obj.userData._originalResolution) {
          obj.userData._originalResolution = obj.material.resolution.clone();
        }
        
        // Reduzir resolução em 50% durante transições
        obj.material.resolution.multiplyScalar(0.5);
      }
      
      // Ocultar objetos muito detalhados temporariamente
      if (obj.name && obj.name.includes("_edges") && obj.name.includes("_thick")) {
        if (!obj.userData._hiddenDuringTransition) {
          obj.userData._hiddenDuringTransition = obj.visible;
          obj.visible = false;
        }
      }
    });
  ...
  // Função para restaurar configurações normais após a transição
  const restoreAfterTransition = () => {
    ...
    sketchGroup.traverse(obj => {
      ...
      if (obj.userData._originalResolution && obj.material && obj.material.resolution) {
        obj.material.resolution.copy(obj.userData._originalResolution);
        delete obj.userData._originalResolution;
        restoreCount++;
      }
      
      // Restaurar visibilidade
      if (obj.userData._hiddenDuringTransition !== undefined) {
        obj.visible = obj.userData._hiddenDuringTransition;
        delete obj.userData._hiddenDuringTransition;
        restoreCount++;
      }
    });
    
    if (restoreCount > 0) {
      debugLog(`Restauradas ${restoreCount} otimizações de transição`);
    }
  };
  ...
  useMemo(() => {
    window._debugSketchMode.getSketchStatus = () => ({
      ...
      cameraTransitionActive: GlobalCache.cameraTransitionActive
    });
  ...
  const updateSketchScene = () => {
    ...
    // Verificar e aplicar otimizações específicas se estamos em transição de câmera
    if (GlobalCache.cameraTransitionActive) {
      simplifyDuringTransition();
    } else {
  ...
  useEffect(() => {
    if (currentMode !== 'normal' || !modelRef.current) return;
    
    // Verificar e corrigir visibilidade a cada segundo
    const visibilityCheckInterval = setInterval(() => {
      if (currentMode === 'normal' && modelRef.current && !modelRef.current.visible) {
        debugLog("Detectada perda de visibilidade do modelo, restaurando...");
        forceModelVisibility(modelRef.current);
      }
    }, 1000);
    
    return () => clearInterval(visibilityCheckInterval);
  }, [currentMode, modelRef.current]);
  ...
  useEffect(() => {
    return () => {
      ...
      try {
        ...
        // Desativar marcador de transição no cache global
        GlobalCache.cameraTransitionActive = false;
