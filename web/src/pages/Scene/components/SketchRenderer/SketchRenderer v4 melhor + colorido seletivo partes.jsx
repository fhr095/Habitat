// SketchRenderer.jsx - Versão Otimizada com Preservação de Materiais (Corrigida)
...
function deepCloneMaterial(material) {
  ...
  // Preservar propriedades específicas de reflexão/iluminação
  if (material.envMapIntensity !== undefined) clone.envMapIntensity = material.envMapIntensity;
  if (material.reflectivity !== undefined) clone.reflectivity = material.reflectivity;
  if (material.refractionRatio !== undefined) clone.refractionRatio = material.refractionRatio;
...
const SketchRenderer = ({ modelRef, scene, camera, renderer, world }) => {
  ...
  const sketchSceneRef = useRef({
    ...
    originalMaterials: new Map(), // Mapa para armazenar materiais originais para preservação
  ...
  useMemo(() => {
    window._debugSketchMode.getSketchStatus = () => ({
      ...
      sketchConfig,
      originalMaterialsCount: sketchSceneRef.current.originalMaterials.size,
      preservedMeshesCount: sketchSceneRef.current.preservedMaterialMeshes.size
    });
  ...
  const captureOriginalMaterial = (mesh) => {
    ...
    if (sketchSceneRef.current.originalMaterials.has(mesh.uuid)) return;
    ...
    try {
      ...
      // Armazenar o material clonado e informações
      sketchSceneRef.current.originalMaterials.set(mesh.uuid, {
  ...
  const createPreservedMaterialMeshes = (sketchGroup) => {
    ...
    const { objectLookup, originalMaterials } = sketchSceneRef.current;
