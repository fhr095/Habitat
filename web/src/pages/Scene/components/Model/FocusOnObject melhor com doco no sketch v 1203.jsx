// FocusOnObject.jsx - Versão Otimizada
...
export function findObjectsByName(targetName, scene) {
  ...
  if (window._debugSketchMode?.findObjectByName) {
    ...
    if (targetMeshes.length > 0) {
      debugLog(`Retornando ${targetMeshes.length} objetos encontrados via API`);
      return filterRelevantObjects(targetMeshes, isSketchMode, targetName);
    }
  }
  ...
  debugLog(`Total de objetos encontrados: ${targetMeshes.length}`);
  return filterRelevantObjects(targetMeshes, isSketchMode, targetName);
}
...
function highlightSketchObjects(scene, targetMeshes, duration) {
  ...
  relatedObjects.forEach(obj => {
    ...
    if (obj.material && obj.material.linewidth !== undefined) {
      new TWEEN.Tween({ linewidth: obj.material.linewidth })
        .to({ linewidth: obj.material.linewidth * 2.5 }, duration)
    ...
    if (obj.material && obj.material.color) {
      ...
      new TWEEN.Tween({ r: orig.r, g: orig.g, b: orig.b })
        .to({ 
          r: Math.min(1, orig.r * 1.8),
          g: Math.min(1, orig.g * 1.8),
          b: Math.min(1, orig.b * 1.8)
        }, duration)
        .easing(TWEEN.Easing.Cubic.Out)
        .onUpdate(v => {
    ...
    if (obj.material && obj.material.uniforms) {
      ...
      if (obj.material.uniforms.diffuse) {
        ...
        new TWEEN.Tween({ 
          ...
          .to({ 
            r: Math.min(1, origColor.r * 1.8),
            g: Math.min(1, origColor.g * 1.8),
            b: Math.min(1, origColor.b * 1.8)
          }, duration)
          .easing(TWEEN.Easing.Cubic.Out)
          .onUpdate(v => {
    ...
    if (obj.material && obj.material.emissive) {
      ...
      new TWEEN.Tween({ 
        ...
        .to({ 
          r: Math.min(1, origEmissive.r + 0.4),
          g: Math.min(1, origEmissive.g + 0.4),
          b: Math.min(1, origEmissive.b + 0.4)
        }, duration)
        .easing(TWEEN.Easing.Cubic.Out)
        .onUpdate(v => {
    ...
    if (!(obj.isLine || obj.isLineSegments)) {
      new TWEEN.Tween({ 
        ...
        .to({ 
          scaleX: obj.scale.x * 1.05,
          scaleY: obj.scale.y * 1.05,
          scaleZ: obj.scale.z * 1.05
        }, duration)
  ...
  // Reduzir opacidade dos objetos não relacionados
  sketchGroup.traverse(obj => {
    if ((obj.isMesh || obj.isLine || obj.isLineSegments || obj.isPoints) && 
      ...
      // Fade para objetos não relacionados
      new TWEEN.Tween({ opacity: obj.material.opacity })
...
export async function focusOnObject(targetName, scene, camera, controls, duration = 3500) {
  return new Promise(async (resolve) => {
    ...
    if (isSketchMode) {
      ...
      // Função padrão de fade para o modo normal
