...
// Cache específico para materiais no modo sketch
const sketchMaterialCache = new Map();
...
/**
 * Anima a transição entre materiais para criar um efeito suave de aparecimento
 * @param {THREE.Object3D} object - O objeto a ter o material trocado
 * @param {THREE.Material} targetMaterial - O material alvo 
 * @param {number} duration - Duração da animação
 */
function animateMaterialTransition(object, targetMaterial, duration) {
  if (!object || !object.material || !targetMaterial) return;
  
  // Salvar material atual no cache para restauração posterior
  sketchMaterialCache.set(object.uuid, {
    currentMaterial: object.material.clone(),
    targetMaterial: targetMaterial
  });
  
  // Preservar propriedades importantes do material atual
  const currentMaterial = object.material;
  if (targetMaterial.color && currentMaterial.color) {
    // Iniciar com a cor atual para transição suave
    const originalColor = targetMaterial.color.clone();
    targetMaterial.color.copy(currentMaterial.color);
    
    // Animar para a cor original
    new TWEEN.Tween({ 
      ...
      .onUpdate(v => {
        targetMaterial.color.setRGB(v.r, v.g, v.b);
        targetMaterial.needsUpdate = true;
      })
      .start();
  }
  
  if (targetMaterial.emissive && currentMaterial.emissive) {
    // Iniciar com emissive atual
    targetMaterial.emissive.copy(currentMaterial.emissive);
  }
  
  // Preservar opacidade e transparência para transição
  if (targetMaterial.opacity !== undefined && currentMaterial.opacity !== undefined) {
    const originalOpacity = targetMaterial.opacity;
    targetMaterial.opacity = currentMaterial.opacity;
    
    // Animar para a opacidade original
    new TWEEN.Tween({ opacity: targetMaterial.opacity })
      .to({ opacity: originalOpacity }, duration)
      .easing(TWEEN.Easing.Cubic.InOut)
      .onUpdate(v => {
        targetMaterial.opacity = v.opacity;
        targetMaterial.needsUpdate = true;
      })
      .start();
  }
...
/**
 * Restaura os materiais originais após o foco
 * @param {THREE.Scene} scene - A cena
 * @param {number} duration - Duração da animação em ms
 */
function restoreSketchMaterials(scene, duration = 1000) {
  if (sketchMaterialCache.size === 0) {
    return;
  }
  
  debugLog(`Restaurando ${sketchMaterialCache.size} materiais sketch originais`);
  
  // Restaurar cada material com animação suave
  sketchMaterialCache.forEach((materialData, uuid) => {
    ...
    if (foundObject && foundObject.material) {
      // Animar a transição de volta ao material sketch original
      const currentMaterial = foundObject.material;
      const originalMaterial = materialData.currentMaterial;
      
      // Animar a opacidade de volta ao normal
      if (currentMaterial.opacity !== undefined && originalMaterial.opacity !== undefined) {
        new TWEEN.Tween({ opacity: currentMaterial.opacity })
          .to({ opacity: originalMaterial.opacity }, duration)
          .easing(TWEEN.Easing.Cubic.InOut)
          .onUpdate(v => {
            currentMaterial.opacity = v.opacity;
            currentMaterial.needsUpdate = true;
          })
          .start();
      }
      
      // Animar cor de volta ao normal
      if (currentMaterial.color && originalMaterial.color) {
        new TWEEN.Tween({ 
          ...
          .to({ 
            r: originalMaterial.color.r,
            g: originalMaterial.color.g,
            b: originalMaterial.color.b 
          }, duration)
          .easing(TWEEN.Easing.Cubic.InOut)
          .onUpdate(v => {
            currentMaterial.color.setRGB(v.r, v.g, v.b);
            currentMaterial.needsUpdate = true;
          })
          .onComplete(() => {
            // Após a animação, restaurar o material original por completo
            foundObject.material = originalMaterial;
            foundObject.material.needsUpdate = true;
          })
          .start();
      } else {
        // Se não tiver cor para animar, apenas restaurar o material
        foundObject.material = originalMaterial;
        foundObject.material.needsUpdate = true;
      }
      
      debugLog(`Material restaurado para ${foundObject.name}`);
    }
  });
  
  // Limpar o cache após um tempo para garantir que as animações terminem
  setTimeout(() => {
    sketchMaterialCache.clear();
    debugLog("Cache de materiais sketch limpo");
  }, duration + 100);
}
...
export function returnToOriginalCamera(originalPosition, originalTarget, scene, camera, controls, duration = 3000) {
  return new Promise((resolve) => {
    ...
    if (isSketchMode) {
      ...
      // Restaurar materiais do sketch
      //restoreSketchMaterials(scene, duration * 0.7);
    } else {
    ...
    let animationTimeoutId = setTimeout(() => {
      ...
      if (isSketchMode) {
        restoreSketchMaterials(scene, 0);
...
/**
 * Extrai o material original de um objeto do cache global do modo sketch
 * @param {THREE.Object3D} object - O objeto para buscar o material original
 * @returns {THREE.Material|null} - O material original ou null se não encontrado
 */
function getOriginalSketchMaterial(object) {
  if (!object) return null;
  
  // Verificar se temos acesso à API do SketchRenderer
  if (window._debugSketchMode && window._debugSketchMode.getOriginalMaterial) {
    const originalData = window._debugSketchMode.getOriginalMaterial(object.uuid);
    if (originalData && originalData.material) {
      debugLog(`Material original para ${object.name} encontrado via API`);
      return originalData.material.clone();
    }
  }
  
  // Verificar se temos acesso ao cache global
  if (window._sketchModeGlobalCache && window._sketchModeGlobalCache.originalMaterials) {
    const originalMaterialMap = window._sketchModeGlobalCache.originalMaterials;
    
    if (originalMaterialMap.has && originalMaterialMap.has(object.uuid)) {
      const origData = originalMaterialMap.get(object.uuid);
      
      if (origData && origData.material) {
        debugLog(`Material original para ${object.name} encontrado via cache global`);
        return origData.material.clone();
      }
    }
  }
  
  // Último recurso: tentar acessar o campo diretamente
  if (window.GlobalCache && window.GlobalCache.originalMaterials) {
    const originalMaterialMap = window.GlobalCache.originalMaterials;
    
    if (originalMaterialMap.has && originalMaterialMap.has(object.uuid)) {
      const origData = originalMaterialMap.get(object.uuid);
      
      if (origData && origData.material) {
        debugLog(`Material original para ${object.name} encontrado via GlobalCache`);
        return origData.material.clone();
      }
    }
  }
...
function highlightSketchObjects(scene, targetMeshes, duration) {
  ...
  sketchGroup.traverse(sketchObj => {
    if (sketchObj.isMesh || sketchObj.isLine || sketchObj.isLineSegments || sketchObj.isPoints) {
      ...
      console.log("HIGHLATED GROUPPPPP:", sketchGroup)
      ...
      for (const targetName of targetNames) {
        ...
        if (
          ...
          // NOVO: Buscar e aplicar material original
          const originalMaterial = getOriginalSketchMaterial(sketchObj);
          if (originalMaterial) {
            animateMaterialTransition(sketchObj, originalMaterial, duration);
            debugLog(`Material original aplicado a ${sketchObj.name}`);
          }
        }
      }
    }
      ...
      if (sketchObj.isMesh || sketchObj.isLine || sketchObj.isLineSegments || sketchObj.isPoints) {
        ...
        for (const targetName of targetNames) {
          ...
          if (objNamePartial.includes(targetPartial) || targetPartial.includes(objNamePartial)) {
            ...
            if (originalMaterial) {
              animateMaterialTransition(sketchObj, originalMaterial, duration);
              debugLog(`Material original aplicado a ${sketchObj.name} (via correspondência flexível)`);
            }
          }
        }
      }
  ...
  // Para objetos que não encontramos material original, aplicar efeitos visuais de destaque
  relatedObjects.forEach(obj => {
    ...
    if (obj.material && obj.material.color && !sketchMaterialCache.has(obj.uuid)) {
...
function animateFade(scene, targetMeshes, toOpacity, duration) {
  ...
  materialsToFade.forEach((obj, material) => {
    ...
    if (targetMeshes.length > 0) {
      ...
      if (minDistance < 5) {
        customOpacity = toOpacity * 0.05;
      } else if (minDistance < 10) {
        customOpacity = toOpacity * 0.07;
      }
    }
...
export async function focusOnObject(targetName, scene, camera, controls, duration = 3500) {
  return new Promise(async (resolve) => {
    ...
    // Restaurar materiais de focos anteriores
    if (isSketchMode && sketchMaterialCache.size > 0) {
      debugLog("Restaurando materiais de focos anteriores");
      restoreSketchMaterials(scene, 300);
    }
    ...
    if (isSketchMode) {
      ...
      console.log("HIGHLATED:", targetMeshes)
