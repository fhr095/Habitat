...
function SceneContent({ habitatId, mainFileUrl, mobileFileUrl }) {
  ...
  // Configuração automática do modo sketch
  const autoConfigureSketchMode = useCallback(() => {
    // Garantir que apenas roda uma vez quando o modelo estiver carregado
    if (model2Loaded && model2Ref.current && !initialSetupDoneRef.current) {
      console.log("Configurando automaticamente o modo sketch...");
      
      // Forçar aplicação do tema light
      setColorTheme('LIGHT');
      
      // Array com os nomes dos objetos a preservar
      const targetObjectNames = [
        'Grama_-_Cidade_Administrativa',
        'Grama_-_Cidade_Administrativa_1',
        'Caminhos_-_Cidade_Administrativa_1',
        'Caminhos_-_Cidade_Administrativa_2'
      ];
      
      // Encontrar os UUIDs dos objetos alvo
      const targetUuids = [];
      model2Ref.current.traverse(obj => {
        if (targetObjectNames.some(name => obj.name && obj.name.includes(name))) {
          targetUuids.push(obj.uuid);
          console.log(`Encontrado objeto alvo: ${obj.name} (UUID: ${obj.uuid})`);
        }
      });
      
      // Adicionar os UUIDs encontrados à lista de preservedObjects
      if (targetUuids.length > 0) {
        console.log(`Adicionando ${targetUuids.length} objetos para preservação de material`);
        // Adicionar cada UUID individualmente para preservação
        targetUuids.forEach(uuid => {
          togglePreservedObject(uuid);
        });
      } else {
        console.warn("Nenhum dos objetos alvo foi encontrado no modelo");
      }
      
      initialSetupDoneRef.current = true;
    }
  }, [model2Loaded, model2Ref, togglePreservedObject, setColorTheme, initialSetupDoneRef]);

  // Efeito para forçar tema light sempre que o modo sketch estiver ativo
  useEffect(() => {
    if (currentMode === 'sketch') {
      // Pequeno atraso para garantir que seja aplicado após a inicialização
      const lightThemeTimeout = setTimeout(() => {
        setColorTheme('LIGHT');
        console.log("Tema light aplicado/reforçado automaticamente");
      }, 100);
      
      return () => clearTimeout(lightThemeTimeout);
    }
  }, [currentMode, setColorTheme]);

  // Efeito para aplicar a configuração automática do modo sketch quando o modelo estiver carregado
  useEffect(() => {
    if (model2Loaded && model2Ref.current) {
      const setupTimeout = setTimeout(() => {
        autoConfigureSketchMode();
      }, 500); // Atraso para garantir que o modelo esteja totalmente processado
      
      return () => clearTimeout(setupTimeout);
    }
  }, [model2Loaded, model2Ref, autoConfigureSketchMode]);
