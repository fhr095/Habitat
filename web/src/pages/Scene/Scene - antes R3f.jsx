...
import LocationLabel from "./components/LocationLabel/LocationLabel"; // Importação do novo componente
...
export default function Scene({ habitatId, mainFileUrl, mobileFileUrl }) {
  ...
  const model2Ref = useRef(null); // Referência para o modelo 2
  ...
  const onLoadModel2 = useCallback((modelScene) => {
    ...
    model2Ref.current = modelScene; // Armazenar a referência do modelo
  ...
  return (
    <div className="scene-container">
      ...
      {console.log(components, world, model2Loaded, model2Ref.current)}
      {components && world && model2Loaded && model2Ref.current && (
        <>
          {console.log("Scene: Rendering LocationLabel", {
            components: !!components,
            world: !!world,
            model2Loaded,
            model2RefExists: !!model2Ref.current
          })}
          <LocationLabel 
            world={world} 
            modelRef={model2Ref} 
