...
export default function ChatGroups() {
  const { habitat, user, chatGroup, setChatGroup } = useHabitatUser(); // Acessando o contexto
  const [messages, setMessages] = useState([]);
  const [newMessage, setNewMessage] = useState("");
  const [members, setMembers] = useState({});

  useEffect(() => {
    const chatRef = doc(db, `habitats/${habitat.id}/groups/${chatGroup.id}`);
    ...
    <div className="chat-groups">
      <header>
        <div className="chat-group-info">
          <img src={chatGroup.imgUrl} alt={chatGroup.name} />
          <div className="text">{chatGroup.name}</div>
        </div>
