...
export default function Scene({ ifcFileUrl, fade, address }) {
  ...
  useEffect(() => {
    if (ifcFileUrl) {
      console.log("Scene useEffect - ifcFileUrl:", ifcFileUrl);
      init();
    }
  }, [ifcFileUrl]);
  ...
  useEffect(() => {
    if (fade !== "" && arrayName.length > 0) {
      console.log("Scene useEffect - fade:", fade);
      arrayName.forEach((obejct) => {
  ...
  async function loadIfc(components, world) {
    ...
    console.log("Scene arrayData:", arrayData);
  }
