...
export default function Scene({ habitatId, mainFileUrl, mobileFileUrl, fade = "", address }) {
  ...
  const { registerHighlightFade } = useSceneActions();
  ...
  useEffect(() => {
    if (mainFileUrl || mobileFileUrl) {
      ...
      const { components, world, controls: orbitControls } = SetupScene(containerRef, setCamera);
  ...
  const highlightFade = async (fadeId) => {
    ...
    if (!world || !world.scene || !camera || !controls) {
        console.log("highlightFade abortado: world, scene, camera ou controls não definidos.");
        return;
    }
    ...
    // Torna todos os objetos transparentes
    ...
    if (targetMeshes.length > 0) {
        ...
        const projectionType = camera.projection?.current || camera.projection?.type || "Perspective";

        let cameraZ;

        if (projectionType === "Perspective" || projectionType === "PerspectiveCamera") {
            const fov = camera.fov || 50; // Valor padrão para FOV
            const aspect = camera.aspect || 1;

            if (!fov || isNaN(fov) || !aspect || isNaN(aspect)) {
                console.error("Valores inválidos de FOV ou aspect:", { fov, aspect });
                restoreOriginalMaterials();
                return;
            }
            ...
            cameraZ = maxDim * 1.5; // Ajuste empírico para o modo ortográfico
        }
...
// No final do Scene.jsx, depois de configurar a cena e o world:
function animate() {
  requestAnimationFrame(animate);
  TWEEN.update();  // Atualiza todos os tweens ativos
  // Caso você tenha outro loop de renderização, este seria o local:
