...
export default function ChatMembers() {
  const { habitat, user, chatMember, setChatMember } = useHabitatUser(); // Agora acessando do contexto
  const [messages, setMessages] = useState([]);
  const [newMessage, setNewMessage] = useState("");

  useEffect(() => {
    const chatId = [user.email, chatMember.email].sort().join("_");
    const chatRef = doc(db, `habitats/${habitat.id}/conversations/${chatId}`);
    ...
    return () => unsubscribe();
  }, [habitat.id, user.email, chatMember.email]);
