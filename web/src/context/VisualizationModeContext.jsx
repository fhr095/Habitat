...
const defaultSketchConfig = {
  colors: 'LIGHT',
  backgroundColor: '#eeeeee', // Valores do LIGHT_THEME
  modelColor: '#ffffff',
  lineColor: '#455A64',
  shadowColor: '#c4c9cb',
  lit: false,
  ...
  thickness: 1,
  useThickLines: false,
  // Ativando preservação de materiais por padrão
  preserveOriginalMaterials: true,
  preservedMaterialTypes: {
    colors: false,
    textures: false,
    metalness: false,
    roughness: false,
...
export const VisualizationModeProvider = ({ children }) => {
  // Estado para o modo atual - iniciar diretamente no modo sketch
  ...
  // Função para atualizar configurações de materiais preservados
  const updatePreservedMaterialTypes = useCallback((type, value) => {
    setSketchConfig(prev => ({
      ...prev,
      preservedMaterialTypes: {
        ...prev.preservedMaterialTypes,
        [type]: value
      }
    }));
  }, []);
  
  // Função para adicionar ou remover objetos da lista de materiais preservados
  const togglePreservedObject = useCallback((objectId) => {
    setSketchConfig(prev => {
      const isCurrentlyPreserved = prev.preservedObjects.includes(objectId);
      
      return {
        ...prev,
        preservedObjects: isCurrentlyPreserved
          ? prev.preservedObjects.filter(id => id !== objectId)
          : [...prev.preservedObjects, objectId]
      };
    });
  }, []);
